...
class STLWriter:
    ...
    def __init__(self, name="object", filename=None, fileobject=None):
        self.stl = ''
        self.name = name
    ...
    def addFacet(self, v1, v2, v3):
        ...
        code += '\tfacet normal ' + ' '.join(map(str, normal)) + '\n'
        code += '\t\touter loop\n'
        code += '\t\t\tvertex ' + ' '.join(map(str, v1)) + '\n'
        code += '\t\t\tvertex ' + ' '.join(map(str, v2)) + '\n'
        code += '\t\t\tvertex ' + ' '.join(map(str, v3)) + '\n'
        code += '\t\tendloop\n'
        code += '\tendfacet\n'
        if self.filename:
            self.f.write(code)
    ...
    def plane(self, v1, v2, v3, v4):
        out = ''
        out += self.addFacet(v1, v2, v3)
        out += self.addFacet(v3, v4, v1)
        #return out
...
if __name__ == '__main__':
    s = STLWriter(filename="cube3.stl")
    s.start()
    #for i in range(1, 100):
    s.addCube(10, 10, 10, rotX=0, rotY=0, rotZ=0, center=[0,0,0])
    s.addCube(10, 10, 10, rotX=0, rotY=0, rotZ=0, center=[10, 0, 5])
    s.addCube(10, 10, 10, rotX=0, rotY=0, rotZ=math.pi/4, center=[-20, 0, 5])
    s.finish()
